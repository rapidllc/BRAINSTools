...
class BAWBIDSFormatter(object):
    ...
    def get_bids_name(self, subject_data: dict, full_path: str = None, ext: str = None) -> str:
        """
        ...
        if self.sub_kw not in subject_data.keys():
            raise KeyError(
                "Subject must have a key of '{kw}' in order to generate a meaningful filename.".format(kw=self.sub_kw)
            )
        if self.ses_kw not in subject_data.keys():
            raise KeyError(
                "Session must have a key of '{kw}' in order to generate a meaningful filename.".format(kw=self.ses_kw)
            )

        # build the bids name
        bids_name = "{subkw}-{subject}_{seskw}-{session}".format(
            subkw=self.sub_kw, subject=subject_data[self.sub_kw], seskw=self.ses_kw, session=subject_data[self.ses_kw]
        )

        for k in sorted(subject_data.keys()):
            if k not in self.REQUIRED_FIELDS:
                bids_name = "{base}_{key}-{value}".format(base=bids_name, key=k, value=subject_data[k])
